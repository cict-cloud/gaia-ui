...
The recommended way to use gaia-ui. Wraps Mantine's `AppShell` and wires together `GaiaHeader`, `GaiaNavbar`, and the mobile burger toggle — you only need to provide props and children.
...
| `children` | `React.ReactNode` | — | Main content rendered inside `AppShell.Main`. |
...
- **No border** — `AppShell` is rendered with `withBorder={false}`; borders between sections are provided by the individual `GaiaHeader` and `GaiaNavbar` components.
- **Padding** — `AppShell.Main` has `padding={20}` applied.
